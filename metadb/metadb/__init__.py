...
class BUFRDesc(Base):
    ...
    _xml_vars =  ('id', 'name', 'description','fn_regex', 'title', 'institution',
            'source', 'history','references','comments', 'bufr_var',
            'bufr_record_start','bufr_record_end')
...
class BUFRVar(Base ):
    ...
    __table_args__ = ( UniqueConstraint('bdesc_id', 'name', 'seq', name='instr_var'), {})
    ...
    def __repr__(self):
        return "<BUFRVar('%s', '%s', '%s')>" % (self.name, self.bdesc_id, self.seq)
...
class BUFRDataType(Base):
    ...
    def get_dtype(self):
        """ returns python type """
        if self.ptype == 'float': return type(0.0)
        if self.ptype == 'int' : return type(0)
        if self.ptype == 'str' : return type('')
        if self.ptype == 'bool': return type(True)
        return None
	
class BUFRParamType(Base):
    """ Defines parameter types, table: 'bufr_param_type'. These entries defines the 
    mappings between BUFR variable names and for instance NetCDF variable names """
    ...
    name = Column(String, unique=True)
    description = Column(String)
    bufr_data_type = relation( BUFRDataType, backref=backref('bufr_param_type', order_by=id))
    ...
    def __repr__(self):
        return "<BUFRParamType('%s','%s','%s')>" % (self.id, self.bufr_data_type_id, self.name)
...
class BUFRParam(Base):
    ...
    bufr_param_type = relation( BUFRParamType , backref=backref('bufr_param', order_by=id))
    
    __table_args__ = ( UniqueConstraint('bufr_var_id', 'bufr_param_type_id', name='distinct_bparam'), {})

    _xml_vars =  ('id', 'bparam_data', 'bufr_var_id', 'bufr_param_type_id','bufr_param_type') 
...
class BUFRDescDBConn(SQLXMLMarshall):
    ...
    def __init__(self, dburl='sqlite:///bufr_var_1.db'):
        """ Initiates singleton connection class. Creates database if not existent"""
        ...
        try:
            ...
            str_type = self._session.query( BUFRDataType ).filter( BUFRDataType.ptype == 'str' ).one()
            float_type = self._session.query( BUFRDataType ).filter( BUFRDataType.ptype == 'float' ).one()
            bool_type = self._session.query( BUFRDataType ).filter( BUFRDataType.ptype == 'bool' ).one()
            int_type = self._session.query( BUFRDataType ).filter( BUFRDataType.ptype == 'int' ).one()
            ...
            self._session.add( BUFRParamType(str_type, "netcdf_global_attribute") )
            self._session.add( BUFRParamType(str_type, "netcdf_missing_value") )
            self._session.add( BUFRParamType(str_type, "netcdf_add_offset") )
            self._session.add( BUFRParamType(float_type, "netcdf__FillValue") )
            self._session.add( BUFRParamType(str_type, "netcdf_long_name") )
            self._session.add( BUFRParamType(float_type, "netcdf_scale_factor") )
            self._session.add( BUFRParamType(str_type, "netcdf_dimension_name") )
            self._session.add( BUFRParamType(int_type, "netcdf_dimension_length") )
    ...
    def get_variable_datatypes(self, instr_name ,var_name):
        """ Returns a list of datatype corresponding to 
        instrument, variable combination"""
        res = []
        var = self._session.query(BUFRVar).join(BUFRDesc).\
                filter( BUFRDesc.name == instr_name and BUFRVar.name == var_name ).one()
        for tdata in var.bufr_param:
            res.append(tdata.bufr_param_type.name)
        return res
    
    def get_variable_param(self, instr_name, var_name, ptype_name):
        """ Returns the parameter data for a defined parameter type and variable"""
        param = self._session.query(BUFRParam).join(BUFRVar).join(BUFRDesc).join(BUFRParamType).\
                filter(BUFRDesc.name == instr_name).filter(BUFRVar.name == var_name).\
                filter(BUFRParamType.name == ptype_name).one()
        return pdata.get_data()


    def get_instrument_variables(self, instr_name):
        """ Returns a list of variable names corresponding to instrument name """
        res = []
        variables = self._session.query(BUFRVar).join(BUFRDesc).filter(BUFRDesc.name == instr_name)
        for v in variables:
            res.append(v.name)
        return res
    ...
    def get_netcdf_parameters(self, instr_name, bufr_var_seq ):
        """ Returns a list of netcdf values corresponding to a bufr variable name 
        
        """
        param = self._session.query(BUFRParam).join(BUFRVar).join(BUFRDesc).join(BUFRParamType).\
                filter(BUFRDesc.name == instr_name).filter(BUFRVar.seq == bufr_var_seq)
        ...
        for p in param:
            ...
            # default guard
            if 'netcdf_' not in p.bufr_param_type.name: 
                continue
    ...
    def get_netcdf_global_attrs(self,instr_name):
        ...
        """
        bparams = self._session.query(BUFRParam).join(BUFRVar).join(BUFRDesc).join(BUFRParamType).\
    ...
    def get_bufr_name(self, instr_name, netcdf_name):
        ...
        """
        bvar = self._session.query(BUFRVar).join(BUFRParam).join(BUFRDesc).join(BUFRParamType).\
                filter(BUFRDesc.name == instr_name).\
    ...
    def get_binary_dump_variables(self,instr_name):
        ...
        """
        params = self._session.query(BUFRParam).join(BUFRVar).join(BUFRDesc).join(BUFRParamType).\
                filter(BUFRDesc.name == instr_name).filter(BUFRParamType.name == 'bin_file')
    ...
    def get_nasa_team_columns(self, instr_name):
        ...
        nasa_low_v = self._session.query(BUFRVar).join(BUFRParam).join(BUFRDesc).join(BUFRParamType).\
                filter(BUFRDesc.name == instr_name).filter(BUFRParamType.name == 'nasa_team_low_V').one()

        nasa_low_h = self._session.query(BUFRVar).join(BUFRParam).join(BUFRDesc).join(BUFRParamType).\
                filter(BUFRDesc.name == instr_name).filter(BUFRParamType.name == 'nasa_team_low_H').one()

        nasa_high_v = self._session.query(BUFRVar).join(BUFRParam).join(BUFRDesc).join(BUFRParamType).\
                filter(BUFRDesc.name == instr_name).filter(BUFRParamType.name == 'nasa_team_high_V').one()
    ...
    def insert_bufr_keys(self, name, fglob, entries, index ):
        ...
        str_type = self._session.query( BUFRDataType ).filter( BUFRDataType.ptype == 'str' ).one()
        bufr_unit = self._session.query( BUFRParamType ).filter( BUFRParamType.name == 'bufr_unit' ).one()
        netcdf_unit = self._session.query( BUFRParamType ).filter( BUFRParamType.name == 'netcdf_unit' ).one()
        netcdf_name = self._session.query( BUFRParamType ).filter( BUFRParamType.name == 'netcdf_name' ).one()
        netcdf_dimension_name = self._session.query( BUFRParamType ).filter( BUFRParamType.name == 'netcdf_dimension_name' ).one()
        netcdf_dimension_length = self._session.query( BUFRParamType ).filter( BUFRParamType.name == 'netcdf_dimension_length' ).one()
        var_type = self._session.query( BUFRParamType ).filter( BUFRParamType.name == 'var_type' ).one()
        netcdf__FillValue = self._session.query( BUFRParamType ).filter( BUFRParamType.name == 'netcdf__FillValue' ).one()
        packable_1dim = self._session.query( BUFRParamType ).filter( BUFRParamType.name == 'packable_1dim' ).one()
        packable_2dim = self._session.query( BUFRParamType ).filter( BUFRParamType.name == 'packable_2dim' ).one()
    	...
    	for entry in entries:
            ...
            # Add dimension name 
            param4 = BUFRParam(netcdf_dimension_name, entry.dimension_name, bvar) 
            self._session.add(param4)

            # Add dimension length
            param5 = BUFRParam(netcdf_dimension_length, entry.dimension_length, bvar) 
            ...
            if 'int' in str(entry.type) or 'long' in str(entry.type) :
                param7 = BUFRParam(netcdf__FillValue, entry.fillvalue_int, bvar) 
                self._session.add(param7)
            elif 'float' in str(entry.type):
                param8 = BUFRParam(netcdf__FillValue, entry.fillvalue_float, bvar) 
